...
http.interceptors.response.use(
  (response) => {
    ...
    if (response.config.url.indexOf('/member/auth/') >= 0 && response.data?.data?.accessToken) {
      ...
      // 登录成功后，立即更新用户数据
      setTimeout(() => {
        console.log('登录成功后，开始强制更新用户数据');
        
        // 强制更新用户数据
        $store('user').updateUserData(true).then(() => {
          console.log('登录成功后立即更新用户数据成功');
          ...
          if (currentPage && currentPage.route === 'pages/index/user') {
            // 强制刷新当前页面
            setTimeout(() => {
          ...
          console.error('登录成功后更新用户数据失败:', error);
        });
      }, 300);
    }
