  ...
  >
    <view class="page-main" :style="[bgMain]">
      ...
      <view class="page-body" :style="[bgBody]">
        ...
        <!-- 底部导航 -->
        <s-tabbar v-if="tabbar === true || tabbar !== ''" :path="typeof tabbar === 'string' ? tabbar : sheep.$router.getCurrentRoute('path')" />
      </view>
    </view>
...
<script setup>
  ...
  // 如果启用了refreshOnShow，则在每次页面显示时触发刷新
  onShow(() => {
    ...
    if (props.tabbar) {
      // 确保导航栏数据已加载
      const appStore = sheep.$store('app');
      if (!appStore.template.basic.tabbar) {
        appStore.getTemplate().then(() => {
          // 触发更新导航栏事件
