...
<script setup>
  ...
  // 监听导航栏更新事件
  let updateNavBarListener = null;
  
  onMounted(() => {
    // 添加导航栏更新事件监听
    updateNavBarListener = () => {
      // 强制组件重新渲染
      if (props.data) {
        // 这里可以添加一些代码来强制更新导航栏样式
        // 例如，可以通过修改data中的某个属性来触发视图更新
        const navbarStyle = document.querySelector('.nav-box');
        if (navbarStyle) {
          navbarStyle.style.opacity = '0.99';
          setTimeout(() => {
            navbarStyle.style.opacity = '1';
          }, 50);
        }
      }
    };
    
    uni.$on('updateNavBar', updateNavBarListener);
  });
  
  onBeforeUnmount(() => {
    // 移除事件监听
    if (updateNavBarListener) {
      uni.$off('updateNavBar', updateNavBarListener);
    }
  });
